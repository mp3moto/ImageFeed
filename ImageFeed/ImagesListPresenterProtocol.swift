...
final class ImagesListPresenter: ImagesListPresenterProtocol {
    ...
    func configCell(for cell: ImagesListCell, with indexPath: IndexPath) {
        let gradient = GradientForView(width: cell.cellImage.bounds.width, height: cell.cellImage.bounds.height, radius: 16).getAnimatedLayer()
        
        if let thumbURL = URL(string: imagesList.photos[indexPath.row].thumbImageURL) {
            cell.cellImage.layer.addSublayer(gradient)
            cell.cellImage.layer.zPosition = 1
            cell.cellImage.kf.setImage(with: thumbURL) { _ in
                cell.cellImage.contentMode = .scaleAspectFill
                gradient.removeFromSuperlayer()
                cell.cellImage.layer.zPosition = 0
            }
        } else {
